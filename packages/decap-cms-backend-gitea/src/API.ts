...
const GITEA_BUILD_MARKER = 'decap-gitea-build-marker-202602031237';
...
export default class API {
  ...
  constructor(config: Config) {
    ...
    if (typeof window !== 'undefined') {
      console.debug(`[decap-cms-backend-gitea] ${GITEA_BUILD_MARKER}`);
    }
  }
  ...
  async persistFiles(dataFiles: DataFile[], mediaFiles: AssetProxy[], options: PersistOptions) {
    ...
    if (options.useWorkflow) {
      ...
      return this.editorialWorkflowGit(files as { path: string; newPath?: string }[], slug, options.collectionName as string, options);
    }
  ...
  async getOpenAuthoringPullRequest(
    ...
    const branchData = await this.getBranch(branch).catch(error => {
      if (
        (error instanceof APIError || error?.name === 'API_ERROR' || error?.name === 'APIError') &&
        error?.status === 404
      ) {
  ...
  async getBranchPullRequest(branchName: string): Promise<GiteaPullRequest> {
    if (this.useOpenAuthoring) {
      ...
      const allPRs = await this.getPullRequests('all');
      const cmsPRs = allPRs.filter(pr => pr.head?.ref?.startsWith(`${CMS_BRANCH_PREFIX}/`));
      return this.getOpenAuthoringPullRequest(branchName, cmsPRs);
    }
  ...
  async createPR(
    ...
    const headRef = await this.getHeadReference(head);
  ...
  async getPullRequestFiles(number: number): Promise<GiteaChangedFile[]> {
    if (typeof window !== 'undefined') {
      console.log(`[decap-cms-backend-gitea] getPullRequestFiles number=${number}`);
    }
    const normalized = this.normalizePullRequestNumber(number);
    if (!normalized) {
      return [];
    }
    return this.request(`${this.originRepoURL}/pulls/${normalized}/files`);
  }
  ...
  async retrieveUnpublishedEntryData(contentKey: string) {
    ...
    if (typeof window !== 'undefined') {
      console.log(
        `[decap-cms-backend-gitea] retrieveUnpublishedEntryData pr=${pullRequest.number}`,
      );
    }
    const normalizedPullRequestNumber = this.normalizePullRequestNumber(pullRequest.number);
    let files: GiteaPullRequestFile[] = [];
    if (normalizedPullRequestNumber) {
      files = await this.getPullRequestFiles(normalizedPullRequestNumber);
    }
  ...
  normalizePullRequestNumber(number: unknown): number | null {
    const asString = String(number);
    if (asString === String(MOCK_PULL_REQUEST) || asString.startsWith('-')) {
      return null;
    }
  ...
  async updateUnpublishedEntryStatus(collection: string, slug: string, newStatus: string) {
    ...
    if (!this.useOpenAuthoring) {
      ...
      if (this.normalizePullRequestNumber(pullRequest.number)) {
        // Real PR exists
        ...
        const diff = await this.getDifferences(this.branch, headRef).catch(() => ({ commits: [] }));
  ...
  async editorialWorkflowGit(
    ...
    if (!unpublished) {
      ...
      const branchExists = await this.getBranch(branch)
        .then(() => true)
        .catch(error => {
          if (
            (error instanceof APIError ||
